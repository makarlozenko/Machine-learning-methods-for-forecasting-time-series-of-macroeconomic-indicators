...
for (i in 1:nrow(unique_combinations_list)) {
  ...
  print(paste("Sukurta lentelę:", table_name))
}
...
financial_accounts_of_general_government <- financial_accounts_of_general_government %>%
  mutate(
    ...
    quarter = as.numeric(str_sub(Time, 6, 6)),       
    time_order = year + (quarter - 1) / 4   
  ) %>%
...
for (i in 1:nrow(unique_combinations_list)) {
  ...
  print(paste("Sukurta lentelė:", table_name))
}
...
for (i in 1:nrow(unique_combinations_list)) {
  ...
  print(paste("Skurta lentelę:", table_name))
}
...
#Funkcija NA šalinimui pagal algoritmą
...
ggplot(final_table, aes(x = factor(Time, levels = unique(Time)), y = BVP)) +
  # 2007–2008 krizė
  ...
  geom_point(color = "#D81B60", size = 2, alpha = 0.8) + 
